			...
			int icl_ua, const char *client)
{
	...
	if (icl_ua <= PL_THRESHOLD_ICL_UA){
		pr_debug("icl_ua <= 900000 disable parallel charger smb1351 \n");
...
//	POWER_SUPPLY_CHARGE_TYPE_TAPER = 4
	if ((pval.intval == 0) || (pval.intval == 4)) {
		...
		pr_debug("chip->charge_type =%d, FORCED pval.intval=%d \n", chip->charge_type,pval.intval);
	} else {
	...
	if ((pval.intval != POWER_SUPPLY_CHARGE_TYPE_FAST)
		...
		pr_debug("main charge CHG_STATE_VOTER disable parallel charging smb1351\n");
...
static void handle_settled_icl_change(struct pl_data *chip)
{
	...
	if (chip->pl_mode == POWER_SUPPLY_PL_USBIN_USBIN) {
		...
		if ((main_limited && (main_settled_ua + chip->pl_settled_ua) < PL_THRESHOLD_ICL_UA)
			...
			pr_debug("total_current_ua <= 900000 disable parallel charger smb1351 \n");
			vote(chip->pl_enable_votable_indirect, USBIN_I_VOTER, false, 0);
