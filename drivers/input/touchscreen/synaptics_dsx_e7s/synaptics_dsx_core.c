...
static void synaptics_rmi4_exp_fn_work(struct kthread_delayed_work *dwork)
{
	struct synaptics_rmi4_exp_fhandler *exp_fhandler;
...
exit:
	...
	if (exp_data.queue_work) {
		kthread_queue_delayed_work(&exp_data.touch_worker,
				&exp_data.work,
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	exp_data.touch_worker_thread = kthread_create(kthread_worker_fn, &exp_data.touch_worker, "dsx_exp_thread");
	...
	kthread_init_delayed_work(&exp_data.work, synaptics_rmi4_exp_fn_work);
...
static int synaptics_rmi4_remove(struct platform_device *pdev)
{
	...
	kthread_cancel_work_sync(&exp_data.work);
	kthread_destroy_worker(&exp_data.touch_worker);
