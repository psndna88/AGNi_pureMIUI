...
ANDROID="Pie"
DEVICE="lavender"
CONFIG="agni_lavender-pie_oldcam_defconfig"
SYNC_CONFIG=1
...
echo ""
echo " ~~~~~ Cross-compiling AGNi Android (Old Cam) $ANDROID kernel $DEVICE ~~~~~"
...
then
	...
	rm -rf $KERNELDIR/$DIR
	echo " <<<<< AGNi Android $ANDROID (Old Cam) has been built for $DEVICE !!! >>>>>>"
	echo "         VERSION: AGNi $AGNI_VERSION_PREFIX $AGNI_VERSION"
	echo "            FILE: $FILENAME"
else
	echo " >>>>> AGNi Android $ANDROID (Old Cam) $DEVICE BUILD ERROR <<<<<"
fi
